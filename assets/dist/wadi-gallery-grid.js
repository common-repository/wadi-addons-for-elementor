...
/*!*****************************!*\
  ...
  \*****************************/
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var isotope_packery__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! isotope-packery */ "../node_modules/isotope-packery/packery-mode.js");
/* harmony import */ var isotope_packery__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(isotope_packery__WEBPACK_IMPORTED_MODULE_0__);
...
/* harmony import */ var _style_gallery_grid_scss__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./style/gallery-grid.scss */ "./src/style/gallery-grid.scss");
...
var WadiGalleryGrid = /*#__PURE__*/function (_elementorModules$fro) {
  _inherits(WadiGalleryGrid, _elementorModules$fro);

  var _super = _createSuper(WadiGalleryGrid);

  function WadiGalleryGrid() {
    _classCallCheck(this, WadiGalleryGrid);

    return _super.apply(this, arguments);
  }

  _createClass(WadiGalleryGrid, [{
    ...
    key: "wadiGalleryGrid",
    value: function wadiGalleryGrid(settings) {
      var _this2 = this;

      var _this = this;

      var galleryWidthString = settings.galleryGridWidth;
      var galleryWidth = galleryWidthString.replace('%', "");
      var galleryWidthNum = parseFloat(galleryWidth);
      var wadiGalleryGrid = this.elements.$wadiGalleryGridElementorContainer;
      var wadiGalleryGridElementsContainers = document.querySelectorAll('.wadi_gallery_grid_container');
      wadiGalleryGridElementsContainers.forEach(function (wadiGalleryGridElement, index) {
        if (wadiGalleryGridElement.querySelector('.wadi_gallery_grid_' + _this2.$element[0].getAttribute("data-id"))) {
          var layoutSettings = {};
          ...
          var iso = new (isotope_layout__WEBPACK_IMPORTED_MODULE_1___default())(wadiGalleryGridElement.querySelector('.wadi_gallery_grid_' + _this2.$element[0].getAttribute("data-id")), {
            // options
          ...
          var elems = iso.getItemElements(); // End checking isotope Elements

          new (imagesloaded__WEBPACK_IMPORTED_MODULE_2___default())(wadiGalleryGridElement.querySelector('.wadi_gallery_grid_' + _this2.$element[0].getAttribute("data-id")), iso); // Shuffle on Page Load
          ...
          var filtersWadiElem = wadiGalleryGridElement.querySelector('.wadi_gallery_grid_filters_list');

          if (filtersWadiElem) {
            var href = window.location.href;
            var url = new URL(href);
            var filterCat = url.searchParams.get(settings.galleryGridParamFilter);
            var itemsList = filtersWadiElem.querySelectorAll("[data-filter]");
            var itemsListArray = Array.from(itemsList);
            filtersWadiElem.addEventListener('click', function (event) {
              return filterWadiGridItems(event);
            });

            var filterWadiGridItems = function filterWadiGridItems(event) {
              ...
              var item = filtersWadiElem.querySelector("[data-filter=\"".concat(filterValue, "\"]"));
            ...
            if ('' !== settings.galleryGridActiveCategory) {
              var activeCategory = settings.galleryGridActiveCategory;
              var activeCategoryTrimed = activeCategory.trim().toLowerCase();
              var cleanActiveCategory = activeCategoryTrimed.replace(" ", "_");

              if (cleanActiveCategory) {
                filtersWadiElem.querySelector("[data-filter=\".".concat(cleanActiveCategory, "\"]")).click();
              }
            } else {
              filtersWadiElem.querySelector("[data-filter=\"*\"]").click();
            }

            var filterAvailable = []; // if(itemsListArray > 0) {

            itemsListArray.forEach(function (item, index) {
              filterAvailable.push(item.dataset.filter);
            }); // }

            if (filterCat !== null && filterCat !== undefined && filterCat && filterAvailable.indexOf(".".concat(filterCat)) > -1) {
              filtersWadiElem.querySelector("[data-filter=\".".concat(filterCat, "\"]")).click();
            }
          }
        }
      });
    ...
    key: "wadiGalleryGridVideos",
    value: function wadiGalleryGridVideos(settings) {
      var _this = this;

      if (settings.galleryGridRepeater.length > 0) {
        var elements = this.getDefaultElements();
        var wadiGalleryGridContainer = elements.$wadiGalleryGrid;
        wadiGalleryGridContainer[0].addEventListener('click', function (e) {
          if (e.target.hasAttribute('data-video-type')) {
            if ('youtube' == e.target.getAttribute('data-video-type') || 'vimeo' == e.target.getAttribute('data-video-type') || 'dailymotion' == e.target.getAttribute('data-video-type')) {
              if (e.target.hasAttribute('data-video-src')) {
                var videoSrcURL = e.target.getAttribute('data-video-src');

                _this.wadiVideoIframe(e.target.parentElement, videoSrcURL);
              }
            } else if ('self_hosted' == e.target.getAttribute('data-video-type')) {
              var _videoSrcURL = e.target.getAttribute('data-video-src');

              var videoParams = e.target.getAttribute('data-self-hosted-settings');
              var jsonVideoParams = JSON.parse(videoParams);

              _this.wadiVideoElement(e.target.parentElement, _videoSrcURL, jsonVideoParams);
            }
          }
        });
      }
    ...
    key: "wadiVideoIframe",
    value: function wadiVideoIframe(girdItemContainerVid, videoSrcURL) {
      var _this = this;
      ...
      _this.wadiHideItemGridElemsForVideo(girdItemContainerVid); // girdItemContainerVid.innerHTML = '';
    ...
    key: "wadiVideoElement",
    value: function wadiVideoElement(girdItemContainerVid, videoSrcURL, videoParams) {
      var _this = this;
    ...
    key: "wadiHideItemGridElemsForVideo",
    value: function wadiHideItemGridElemsForVideo(gridItem) {
      var gridItemChildren = gridItem.children;

      var gridItemChildrenArr = _toConsumableArray(gridItemChildren);

      gridItemChildrenArr.forEach(function (child, index) {
        child.classList.add('hide_grid_item_thumb');
      });
    }
  }]);

  return WadiGalleryGrid;
...
jQuery(window).on("elementor/frontend/init", function () {
  ...
  elementorFrontend.hooks.addAction("frontend/element_ready/wadi-gallery-grid-addon.default", addHandler);
});
}();
/******/ })()
;
//# sourceMappingURL=wadi-gallery-grid.js.map